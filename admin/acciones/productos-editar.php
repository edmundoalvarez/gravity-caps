...
if(!$producto) {
    $_SESSION['mensajeError'] = 'El producto que buscabas no existe' . $e->getMessage();
...
try {
    (new Producto)->editar($id, [
        ...
        'imagen_grande_01'      =>$nombreImgGrande01 ?? $producto->getImgGrande1(),
        'precio'                =>$precio,
        'stock'                 =>$stock,
    ...
    if(isset($nombreImgGrande01) && $producto->getImgGrande1() !== null){
    ...
    $_SESSION['mensajeExito'] = "El producto <b>" . $nombre . "</b> fue editado con éxito.";
    header("Location: ../index.php?s=productos");
    exit;

} catch(Exception $e) {

    $_SESSION['mensajeError'] = 'Ocurrió un problema inesperado al tratar de editar el producto. Probá de nuevo mas tarde. Si el problema persiste, comunicate con nosotros.';
    $_SESSION['oldData'] = $_POST;
    header("Location: ../index.php?s=producto-editar&id=" . $id);
    exit;
