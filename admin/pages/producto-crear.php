...
<section class="crear-producto-completa">
    ...
    <div class="crear-producto-div-form">
        <form action="acciones/productos-crear.php" method="post" enctype="multipart/form-data">
            ...
            <div class="crear-producto-fk">
                <div>
                    ...
                    <select 
                        ...
                        id="categoria_fk" 
                    >
                    <option value="no-seleccion" selected disabled>Seleccionar una categoría*</option>
                    <?php 
                    foreach($categoria as $c): 
                    ?>
                    <option 
                        value="<?= $c->getCategoriaId();  ?>" 
                        <?= $c->getCategoriaId() == ($oldData['categoria_fk'] ?? null) ? 'selected': '';?>
                    > 
                        <?= $c->getNombre(); ?>
                    </option>
                    <?php 
                ...
                <div>
                    ...
                    <select 
                        ...
                        id="subcategoria_fk" 
                    >
                    <option value="no-seleccion" selected disabled>Seleccionar una subcategoria*</option>
                    <?php 
                    foreach($subcategoria as $sc): 
                    ?>
                    <option 
                        value="<?= $sc->getSubcategoriaId(); ?>" 
                        <?= $sc->getSubcategoriaId() == ($oldData['subcategoria_fk'] ?? null) ? 'selected': '';?>
                    > 
                        <?= $sc->getNombre(); ?>
                    </option>
                    <?php 
                ...
                <div>
                    ...
                    <select 
                        ...
                        id="talle_fk" 
                    >
                    <option value="no-seleccion" selected disabled>Seleccionar una talle*</option>
                    <?php 
                    foreach($talle as $t): 
                    ?>
                    <option 
                        value="<?= $t->getTalleId(); ?>" 
                        id="<?= $t->getTalleId(); ?>"
                        <?= $t->getTalleId() == ($oldData['talle_fk'] ?? null) ? 'selected': '';?>
                    > 
                        <?= $t->getNombre(); ?>
                    </option>
                    <?php 
                            ...
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input
                                        ...
                                        <?= isset($oldData['color_fk']) && in_array($color->getColorId(), $oldData['color_fk']) ? 'checked' : '';?>
                                    >
                                    <?= $color->getNombre();?>
                                </label>
                            </div>
            ...
            <div class="crear-producto-img">
                <div>
                    <label for="imagen_grande_01" class="form-label">Imagen grande*</label>
                    <input 
                    class="form-control" 
                    type="file" 
                    id="imagen_grande_01"
                    name="imagen_grande_01"
                    <?php if(isset($errores['imagen_grande_01'])): ?> aria-describedby="error-imagen_grande_01" <?php endif; ?>
                    >
                    <?php 
                        if(isset($errores['imagen_grande_01'])):
                    ?>
                        <div data-error="form">
                            <p id="error-imagen_grande_01" class="form-text"> <?= $errores['imagen_grande_01'] ?> </p>
                        </div>
                    <?php endif;?>                
                </div>
            ...
            <div>
                ...
                <input 
                    ...
                    value="<?= $oldData['imagen_descripcion'] ?? null ?>"
            ...
            <div>
                ...
                >
                <option value="no-seleccion" selected disabled>Seleccionar estado de publicación*</option>
                <?php 
                foreach($estadosPublicacion as $estado): 
                ?>
                <option 
                    value="<?= $estado->getEstadoPublicacionId(); ?>" 
                    <?= $estado->getEstadoPublicacionId() == ($oldData['estado_publicacion_fk'] ?? null) ? 'selected' : ''; ?>
                > 
                    <?= $estado->getNombre(); ?>
                </option>
                <?php 
            ...
            <div>
                <button class="btn-crear-producto" type="submit">Crear producto</button>
            </div>
