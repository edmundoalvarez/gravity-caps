<?php 
    ...
    $productos = (new Producto)->todos($busqueda);
...
<section class="productos-completa">
    ...
    <div class="productos-filtro">
        <h3>Filtros:</h3>
        <a href="index.php?s=productos" class="filtro-productos-todos" data-productos="filtro-seleccionado">Ver todos</a>
        <?php foreach($subcategoria as $s): ?>
            <a href="index.php?s=productos-genero&g=<?= $s->getSubcategoriaId(); ?>" class="filtro-productos-todos"> <?= $s->getNombre()?> </a>
        <?php endforeach; ?>
    </div>
